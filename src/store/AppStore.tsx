...
import { BackupPayload, CloudSession, isCloudConfigured, pullBackup, pushBackup, signIn as cloudSignIn, signUp as cloudSignUp } from '@/src/services/cloud';
...
export function AppStoreProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => { AsyncStorage.getItem(STORAGE_KEY).then((raw) => { if (raw) applyPayload(JSON.parse(raw)); }).finally(() => setReady(true)); }, []);
  ...
  const value = useMemo<AppState>(() => ({ selectedTcg, language, region, collection, wishlist, savedCards, session, cloudConfigured: isCloudConfigured, syncState, syncError, lastSyncedAt, autoSyncEnabled, selectTcg, setLanguage, setRegion,
    ...
    signIn: login, signUp: register, signOut: () => { setSession(null); setAutoSyncEnabled(false); setSyncState('idle'); setSyncError(null); }, backupNow: () => performBackup(), restoreBackup: restore,
