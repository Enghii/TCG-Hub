...
export type CloudSession = { accessToken: string; refreshToken: string; user: { id: string; email: string } };
...
function toSession(body: any): CloudSession { return { accessToken: body.access_token, refreshToken: body.refresh_token, user: { id: body.user.id, email: body.user.email ?? '' } }; }
