...
import { formatsFor } from '@/src/services/deckRules';
...
export default function DecksScreen() {
  ...
  return <SafeAreaView edges={['bottom']} style={styles.safe}><ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
    ...
    <Text style={styles.section}>Meus decks</Text>{items.length === 0 ? <Text style={styles.empty}>Nenhum deck criado ainda.</Text> : <View style={styles.list}>{items.map((deck) => { const total = deck.cards.reduce((sum, item) => sum + item.quantity, 0); const format = formatsFor(deck.tcgId).find((item) => item.id === deck.format); return <Pressable key={deck.id} onPress={() => router.push({ pathname: '/deck/[id]', params: { id: deck.id } })} style={({ pressed }) => [styles.deck, pressed && pressFeedback]}><View style={styles.deckIcon}><Text style={styles.deckIconText}>▤</Text></View><View style={{ flex: 1 }}><Text style={styles.deckName}>{deck.name}</Text><Text style={styles.deckMeta}>{format?.label ?? 'Livre'} · {total} cartas · {deck.cards.length} diferentes</Text></View><Text style={styles.arrow}>→</Text></Pressable>; })}</View>}
