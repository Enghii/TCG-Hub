...
export default function CollectionEntryScreen() {
  ...
  const card = cards.find((item) => item.id === cardId); const existing = collection.find((item) => item.id === entryId);
  ...
  if (!card) return null;
  const save = () => { const parsedQuantity = Math.max(1, Math.floor(Number(quantity) || 1)); const parsedPrice = purchasePrice.trim() === '' ? null : Math.max(0, Number(purchasePrice.replace(',', '.')) || 0); const draft = { cardId: card.id, quantity: parsedQuantity, condition, language: cardLanguage, foil, purchasePrice: parsedPrice }; if (existing) updateCollectionItem(existing.id, draft); else addCollectionItem(draft); router.back(); };
